...
# Integrated Ethernet Diagnostic Script
...
# Function to detect if the system is Silverblue
is_silverblue() {
    if [ -f /etc/os-release ]; then
        . /etc/os-release
        if [[ "$VERSION" == *"Silverblue"* ]]; then
            return 0
        fi
    fi
    return 1
...
install_packages() {
    ...
    case $distro in
        ubuntu | debian)
            ...
            sudo apt-get install -y -qq network-manager iproute2 speedtest-cli usbutils ethtool inxi bc net-tools pciutils
            ;;
        fedora | centos | rhel | bazzite)
            if ! is_silverblue; then
                sudo dnf install -y -q NetworkManager iproute speedtest-cli usbutils ethtool inxi bc net-tools pciutils
            else
                log_and_display_plain "Silverblue detected. Skipping package installation."
            fi
            ;;
        *)
...
summarize_findings() {
    ...
    if [[ -n "$ping" && -n "$download" && -n "$upload" ]]; then
        ...
        if (( $(echo "$download > 100" | bc -l) )); then
            log_and_display_plain "- Fast download speed (${download} Mbit/s). Excellent for streaming, large file downloads, and multiple users."
        elif (( $(echo "$download > 25" | bc -l) )); then
            log_and_display_plain "- Good download speed (${download} Mbit/s). Suitable for most online activities and HD streaming."
        else
            log_and_display_plain "- Lower download speed (${download} Mbit/s). You might experience buffering with HD streaming or slow file downloads."
        fi
...
log_and_display "Integrated Ethernet Diagnostic Script"
log_and_display "-------------------------------------"
...
if [[ "$eth_status" == "connected" ]]; then
    log_and_display_plain "• Your Ethernet connection is active and functioning."
...
if [ "$VPN_ACTIVE" == "Yes" ]; then
    log_and_display_plain "• Note that your active VPN ($VPN_TYPE) may impact your internet speeds and latency."
fi

log_and_display_plain "• For detailed diagnostics, please review the log file: ${YELLOW}$LOGFILE${RESET}"
